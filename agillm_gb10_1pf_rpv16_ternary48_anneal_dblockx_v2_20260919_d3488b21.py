...
NAME = "AGILLM-GB10-1PF-ternary48-anneal-dblockx-v2"
...
# Experimental low-bit branch.  "binary48" keeps the hardware-legal pair-4:8
# sparsity mask, but constrains every surviving connection to +/- one row scale.
# That makes the effective sparse weight alphabet {-a, 0, +a}: binary sign on
# live connections plus structural zero.  Master weights and optimizer state
# remain FP32; the forward/backward shadow is STE-like, exactly as the existing
# NVFP4 shadow already is.
LOWBIT_MODE = os.environ.get("AGILLM_LOWBIT_MODE", "off").strip().lower()
if LOWBIT_MODE not in ("off", "binary48", "ternary158"):
    raise ValueError(f"unsupported AGILLM_LOWBIT_MODE={LOWBIT_MODE!r}")
try:
    LOWBIT_ALPHA = float(os.environ.get("AGILLM_LOWBIT_ALPHA", "1.0"))
except ValueError:
    raise ValueError("AGILLM_LOWBIT_ALPHA must be numeric")
if not math.isfinite(LOWBIT_ALPHA) or not 0.0 <= LOWBIT_ALPHA <= 1.0:
    raise ValueError("AGILLM_LOWBIT_ALPHA must be in [0,1]")

@torch.no_grad()
def lowbit_sparse_shadow(weight, fixed_mask):
    masked = weight * fixed_mask
    if LOWBIT_MODE == "off" or LOWBIT_ALPHA <= 0.0:
        return masked
    live = fixed_mask.to(dtype=weight.dtype)
    denom = live.sum(dim=1, keepdim=True).clamp_min_(1.0)
    scale = (masked.abs().sum(dim=1, keepdim=True) / denom).clamp_min_(2.0 ** -24)
    if LOWBIT_MODE == "binary48":
        sign = torch.where(masked >= 0, torch.ones_like(masked), -torch.ones_like(masked))
        target = sign * scale * live
    else:
        # BitNet-style ternary target {-scale,0,+scale} on the hardware 4:8 support.
        target = torch.round(masked / scale).clamp_(-1.0, 1.0) * scale * live
    if LOWBIT_ALPHA >= 1.0:
        return target
    # Continuation/QAT homotopy: begin from the trained sparse FP master shadow
    # and move toward the low-bit codebook without the abrupt CE cliff seen by v1.
    return masked.lerp(target, LOWBIT_ALPHA)
...
class SparseLinear(nn.Module):
    ...
    def shadow(self, n):
        ...
        if self._cache_key != key:
            masked = lowbit_sparse_shadow(self.weight, self._fixed_mask)
...
class Expert(nn.Module):
    ...
    def forward(self, x):
        ...
        if not gate.requires_grad and not up.requires_grad and not self.down.weight.requires_grad:
...
class Stage(nn.Module):
    ...
    def forward(self, x):
        ...
        if STAGEFWD["syncfree_routing"] and flat.shape[0] % EXPERTS == 0:
            ...
            y = _ScatterRowsFn.apply(order, groups, *ys)
...
def _rpv16_ce_row_kernel(
    ...
    # (ii) masked group log-softmax, FP32 accumulators. exp(-1e9 - max) == 0 in FP32, so -inf is identical.
    g = tl.load(gptr).to(tl.float32)
    g = tl.where(gi[None, :] < NG, g, float("-inf"))
    ...
    tl.store(NLL + row, -(tmax + tl.log(tsum)))
    ...
    if WITH_GRAD:
        ...
        ws = w * scale
        dg = ws[:, None] * (ge / gsum[:, None] - tl.where(g_hit, 1.0, 0.0))   # padded classes -> exactly 0
...
def rpv16_fused_ce(hidden_rows, labels, factor_weight, mix_weight, ce_chunk=4096, reduction="mean",
    ...
    return _RPV16FusedCE.apply(hh, yy, factor_weight, mix_weight, int(ce_chunk), reduction == "mean", hint)
...
class Model(nn.Module):
    ...
    def set_trainable_stage(self, stage_idx=None, head_anchor=False, dblock_subphase=-1):
        # Experimental DBlock-X: in subcycle mode each visit updates only part of
        # the active stage.  0=gate/up(+router,n2), 1=down only, 2=full-stage
        # quality anchor.  The old whole-stage behavior remains the default.
        for p in self.parameters():
            p.requires_grad_(False)
        for st in self.stages:
            st._dblock_subphase = -1
        if head_anchor:
            ...
            st = self.stages[int(stage_idx)]
            sub = int(dblock_subphase)
            st._dblock_subphase = sub
            if sub < 0:
                for p in st.parameters(): p.requires_grad_(True)
                if int(stage_idx) == 0:
                ...
                st._dblock_subphase = sub
                if sub == 0:
                    # Gate/up need dA through frozen down, but avoid down dW and
                    # all attention dW. Router+n2 remain trainable for routing.
                    for mod in (st.n2, st.router):
                        for p in mod.parameters(): p.requires_grad_(True)
                    for e in st.experts:
                        for mod in (e.gate, e.up):
                            for p in mod.parameters(): p.requires_grad_(True)
                elif sub == 1:
                    # Down-only is especially cheap: frozen gate/up run their
                    # direct packed no-grad path; only down receives dW.
                    for e in st.experts:
                        for p in e.down.parameters(): p.requires_grad_(True)
                else:
                    # Periodic full-stage anchor prevents permanently starving
                    # attention/norms and lets stage 0 refresh token input maps.
                    for p in st.parameters(): p.requires_grad_(True)
                    if int(stage_idx) == 0:
                        for mod in (self.embed, self.in_proj):
                            for p in mod.parameters(): p.requires_grad_(True)
    ...
    def sparse_telemetry(self):
        ...
        return {"layers":len(xs),"kernel_calls":sum(m.kernel_calls for m in xs),"packs":sum(m.pack_count for m in xs),"lowbit_mode":LOWBIT_MODE,"lowbit_alpha":LOWBIT_ALPHA}
...
class HFStream:
    ...
    def _raw_ids(self,name):
        # Finite HF sample configs (e.g. fineweb-edu sample-10BT) eventually hit
        # StopIteration. For multi-hundred-B runs, wrap to epoch N+1 from the start
        # of that source instead of permanently disabling it (which crashloops the
        # keeper once the saved cursor is near the end of the sample).
        wraps = 0
        while True:
...
def train(args):
    ...
    for step in range(start_step,args.steps+1):
        if micro_in_update == 0:
            ...
            dblock_subphase = -1
            if (not global_anchor) and int(getattr(args, "dblock_subcycle", 0)) > 0:
                # Rotate sublayer phase once per complete 16-stage+head cycle,
                # so every stage sees the same subphase in a given macro-cycle.
                dblock_subphase = (optimizer_update_clock // (STAGES + 1)) % min(3, int(args.dblock_subcycle))
            model.set_trainable_stage(train_stage, head_anchor=global_anchor, dblock_subphase=dblock_subphase)
            if global_anchor:
        ...
        rec={"event":"train","step":step,"seen_tokens":seen,"loss":loss_v,"aux":aux_v,"grad_norm":gn_v,"optimizer_step":do_update,"grad_accum":args.grad_accum,"optimizer_update_clock":optimizer_update_clock,"micro_in_update":micro_in_update,"train_stage":train_stage,"dblock_subphase":(None if global_anchor else int(getattr(model.stages[int(train_stage)], "_dblock_subphase", -1))),"lowbit_mode":LOWBIT_MODE,"lowbit_alpha":LOWBIT_ALPHA,"global_anchor":global_anchor,"global_anchor_every":args.global_anchor_every,"lr":float(active_opt.param_groups[0]["lr"]),"optimizer":optimizer_name,"step_s":dt,"tok_s":None if dt is None else args.batch*args.seq/dt,"sources":srcs,"sparse":tel,"cuda_alloc":torch.cuda.memory_allocated(),"cuda_reserved":torch.cuda.memory_reserved(),"route_min":min(min(x) for x in counts),"route_max":max(max(x) for x in counts)}
...
def main():
    ...
    t.add_argument("--dblock-subcycle",type=int,default=0,help="experimental DBlock-X: 0=off, 3=gate/up -> down -> full-stage macro-cycle")
