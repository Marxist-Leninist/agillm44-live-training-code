...
def lowbit_sparse_shadow(weight, fixed_mask):
    ...
    if LOWBIT_MODE == "binary48":
        ...
        return sign * scale * live
    # BitNet-style ternary shadow on the already hardware-sparse support.
    q = torch.round(masked / scale).clamp_(-1.0, 1.0)
    return q * scale * live
...
class Model(nn.Module):
    ...
    def sparse_telemetry(self):
        ...
        return {"layers":len(xs),"kernel_calls":sum(m.kernel_calls for m in xs),"packs":sum(m.pack_count for m in xs),"lowbit_mode":LOWBIT_MODE}
...
def train(args):
    ...
    for step in range(start_step,args.steps+1):
        ...
        rec={"event":"train","step":step,"seen_tokens":seen,"loss":loss_v,"aux":aux_v,"grad_norm":gn_v,"optimizer_step":do_update,"grad_accum":args.grad_accum,"optimizer_update_clock":optimizer_update_clock,"micro_in_update":micro_in_update,"train_stage":train_stage,"dblock_subphase":(None if global_anchor else int(getattr(model.stages[int(train_stage)], "_dblock_subphase", -1))),"lowbit_mode":LOWBIT_MODE,"global_anchor":global_anchor,"global_anchor_every":args.global_anchor_every,"lr":float(active_opt.param_groups[0]["lr"]),"optimizer":optimizer_name,"step_s":dt,"tok_s":None if dt is None else args.batch*args.seq/dt,"sources":srcs,"sparse":tel,"cuda_alloc":torch.cuda.memory_allocated(),"cuda_reserved":torch.cuda.memory_reserved(),"route_min":min(min(x) for x in counts),"route_max":max(max(x) for x in counts)}
