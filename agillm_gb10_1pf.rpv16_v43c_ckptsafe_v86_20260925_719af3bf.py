        ...
        if do_update:
            ...
            if train_stage == JOINT_STAGE:
                ...
                if (RPV16_ALWAYS_HEAD and (not global_anchor) and train_stage != JOINT_STAGE and _ah_obj == "ar"):
                    for pg in head_opt.param_groups: pg["lr"] = _lr_use * RPV16_HEAD_LR_MULT
            ...
            _due_updates = bool(args.save_every_updates and update_number % args.save_every_updates == 0)
            _due_time = (_ckpt_every_sec > 0 and time.time() - ckpt_state["last_save_wall"] >= _ckpt_every_sec)
            if _due_updates or _due_time:
                ...
                # V42: success or failure, the next wall-clock attempt is one interval later (no per-step retry storm).
