...
class Model(nn.Module):
    ...
    def forward(self, ids, labels=None, train_stage=None, head_anchor=False):
        ...
        if head_anchor:
            ...
            ts = int(train_stage)
            _sw = getattr(self, "_descending_sweep_cache", None)
            if ts == 0:
                x = self.in_proj(self.embed(ids))
            elif _sw is not None:
                if _sw["objective"] != "ar":
                    raise RuntimeError("AR local forward saw a non-AR sweep cache")
                x, aux_total, route_counts = _sw["prefixes"][ts]
                x = x.detach(); aux_total = aux_total.detach()
                route_counts = [list(c) for c in route_counts]
            else:
                with torch.no_grad():
                    x = self.in_proj(self.embed(ids))
                    for j in range(ts):
...
def ah_trunk(model, ids, ts):
    ...
    route_counts = []
    ts = int(ts)
    _sw = getattr(model, "_descending_sweep_cache", None)
    if ts == 0:
...
class AllHeadsRuntime:
    ...
    def _forward_nat(self, model, ids, labels, ts, clock, micro):
        _sw = getattr(model, "_descending_sweep_cache", None)
        if (_sw is not None and _sw.get("objective") == "nat"
                and "nat_realization" in _sw):
            model_ids, mask, stats = _sw["nat_realization"]
        else:
...
@torch.no_grad()
def _descending_cache_core(model, model_ids, objective):
    prefixes = {}
    aux_total = torch.zeros((), device=model_ids.device, dtype=torch.float32)
    route_counts = []
    x = model.in_proj(model.embed(model_ids))
    # Prefix for stage 15 is the output of stage 14.  The old stage-15 output
    # is never consumed by the descending local updates, so do not evaluate it.
    for j in range(STAGES - 1):
        if j > 0:
            prefixes[j] = (x.detach(), aux_total.detach(),
                           tuple(tuple(int(v) for v in c) for c in route_counts))
        x, aux, counts = model.stages[j](x)
        aux_total = aux_total + aux
        route_counts.append(counts)
    prefixes[STAGES - 1] = (x.detach(), aux_total.detach(),
                            tuple(tuple(int(v) for v in c) for c in route_counts))
    return {"objective": objective, "prefixes": prefixes}

@torch.no_grad()
def build_descending_sweep_cache(model, ids, objective, AH, clock):
    if objective == "ar":
        return _descending_cache_core(model, ids, objective)
    if AH is None:
        raise RuntimeError("non-AR descending cache requires AllHeadsRuntime")
    if objective == "sat":
        eff = AH.effective()
        B, T = ids.shape
        gen = ah_generator(AH.seed, int(clock), 2, 0)
        first2, _, _, _ = (x.expand(B, T) for x in
                            ah_sat_partition(1, T, eff["p2"], AH._fixed2, gen))
        with ah_attn_mode("sat", first2.to(ids.device)):
            return _descending_cache_core(model, ids, objective)
    if objective == "nat":
        gen = ah_generator(AH.seed, int(clock), 3, 0)
        model_ids, mask, stats = ah_nat_corrupt(ids, AH.cfg, gen)
        if AH.nat_attn == "bidir":
            with ah_attn_mode("nat_bidir"):
                cache = _descending_cache_core(model, model_ids, objective)
        else:
            cache = _descending_cache_core(model, model_ids, objective)
        # Carry the exact realized corruption into _forward_nat.  Regenerating an
        # equivalent random mask is unnecessary work and can perturb CUDA
        # allocation/execution ordering across the A/B even with a private CPU RNG.
        cache["nat_realization"] = (model_ids.detach(), mask.detach(), stats)
        return cache
    raise ValueError(f"unsupported sweep objective {objective!r}")

def train(args):
    sweep_enabled = bool(getattr(args, "descending_sweep_shared_batch", False)
                         or getattr(args, "descending_sweep_cache", False)
                         or getattr(args, "descending_sweep_safe_cache", False))
    if sweep_enabled:
        if args.grad_accum != 1:
            raise ValueError("descending sweep v1 requires grad-accum=1")
        # Production saves are deferred to the end of a complete 16-stage + head sweep.
        # An incomplete sweep is never serialized because its reused batch/objective/cache
        # are intentionally ephemeral and are not part of the checkpoint schema.
    ...
    active_opt = None
    sweep_batch = None
    sweep_objective = None
    sweep_clock = None
    sweep_fresh_tokens = 0
    sweep_exposure_tokens = 0
    sweep_stage_evals = 0
    periodic_save_pending = False
    periodic_save_due_clock = None
    next_periodic_save_clock = (
        ((optimizer_update_clock // args.save_every_updates) + 1) * args.save_every_updates
        if args.save_every_updates else None
    )
    active_params = None
    ...
    def _checkpoint(step,seen_tokens,force_sync=False,reason="periodic"):
        ...
        if sweep_enabled:
            payload["descending_sweep"] = {
                "schema": "agillm.descending-sweep.production.v1",
                "shared_batch": True,
                "safe_cache": bool(getattr(args, "descending_sweep_safe_cache", False)),
                "cache_policy": "ar_sat_cached_nat_uncached",
                "checkpoint_boundary": "after_head_phase",
            }
        ...
        if sweep_enabled:
            event.update({"descending_sweep_safe_boundary": True, "descending_sweep_safe_cache": bool(getattr(args, "descending_sweep_safe_cache", False))})
    ...
    if tel2:
        _emit({"event":"fullstack_config","ts":time.time(),"version":"v2_fullstack_20260919","prefetch":not args.no_prefetch,"prefetch_depth":args.prefetch_depth,"prefetch_pin":not args.no_prefetch_pin,"async_checkpoint":not args.sync_checkpoint,"async_ckpt_reserve_gb":args.async_ckpt_reserve_gb,"ckpt_fsync":not args.no_ckpt_fsync,"signal_save":not args.no_signal_save,"lazy_step_sync":lazy,"finite_check_every":args.finite_check_every,"descending_sweep_shared_batch":sweep_enabled,"descending_sweep_safe_cache":bool(getattr(args,"descending_sweep_safe_cache",False)),"descending_sweep_cache_policy":("ar_sat_cached_nat_uncached" if getattr(args,"descending_sweep_safe_cache",False) else None),"checkpoint_policy":("defer_to_head_boundary" if sweep_enabled and args.save_every_updates else "normal"),"proc_uptime_s":time.time()-_PROC_T0})
    ...
    for step in range(start_step,args.steps+1):
        ...
        fetched_fresh = True
        if (sweep_enabled
                and sweep_batch is not None and phase != 0):
            ids, labels = sweep_batch
            srcs = {"sweep_replay": 1}
            fetched_fresh = False
            ...
            data_wait_s=time.perf_counter()-t_fetch
            if sweep_enabled:
                sweep_fresh_tokens += args.batch * args.seq
                if phase == 0:
                    sweep_batch = (ids, labels)
                    sweep_clock = int(optimizer_update_clock)
        ...
        if sweep_enabled and sweep_batch is not None:
            if global_anchor:
                _ah_obj = "ar"
                model._descending_sweep_cache = None
            else:
                if phase == 0:
                    sweep_objective = "ar" if AH is None else AH.objective_for(sweep_clock, 0)
                    _cache_this_objective = (
                        bool(getattr(args, "descending_sweep_cache", False))
                        or (bool(getattr(args, "descending_sweep_safe_cache", False))
                            and sweep_objective in ("ar", "sat")))
                    if _cache_this_objective:
                        model._descending_sweep_cache = build_descending_sweep_cache(
                            model, ids, sweep_objective, AH, sweep_clock)
                        sweep_stage_evals += STAGES - 1
                    else:
                        model._descending_sweep_cache = None
                _ah_obj = sweep_objective
            _obj_clock = sweep_clock
        else:
            model._descending_sweep_cache = None
            _ah_obj="ar" if (AH is None or global_anchor) else AH.objective_for(optimizer_update_clock,micro_in_update)
            _obj_clock = optimizer_update_clock
        if _ah_obj == "ar":
            loss,_,aux,counts=model(ids,labels,train_stage=train_stage,head_anchor=global_anchor)
        else:
            loss,aux,counts=AH.forward(_ah_obj,model,ids,labels,train_stage,_obj_clock,micro_in_update)
        if sweep_enabled:
            if global_anchor:
                sweep_stage_evals += STAGES
            elif sweep_batch is not None and model._descending_sweep_cache is not None:
                sweep_stage_evals += 1
            else:
                sweep_stage_evals += int(train_stage) + 1
        ...
        if do_update:
            ...
            if sweep_enabled:
                sweep_exposure_tokens += args.batch * args.seq
                if global_anchor and sweep_batch is not None:
                    model._descending_sweep_cache = None
                    sweep_batch = None
                    sweep_objective = None
                    sweep_clock = None
            if next_periodic_save_clock is not None and update_number >= next_periodic_save_clock:
                periodic_save_pending = True
                periodic_save_due_clock = next_periodic_save_clock
                while next_periodic_save_clock <= update_number:
                    next_periodic_save_clock += args.save_every_updates
                if sweep_enabled and not global_anchor:
                    _emit({"event":"checkpoint_deferred_sweep","ts":time.time(),"step":step,"seen_tokens":seen + args.batch*args.seq,"optimizer_update_clock":update_number,"due_clock":periodic_save_due_clock,"phase":phase,"save_at":"next_head_boundary"})
            checkpoint_safe_boundary = (not sweep_enabled) or (global_anchor and sweep_batch is None)
            if periodic_save_pending and checkpoint_safe_boundary:
                if lazy:
                ...
                saved_ok = _checkpoint(step,seen + args.batch*args.seq)
                _emit({"event":"checkpoint_safe_boundary","ts":time.time(),"step":step,"seen_tokens":seen + args.batch*args.seq,"optimizer_update_clock":update_number,"due_clock":periodic_save_due_clock,"saved":bool(saved_ok),"phase":phase,"global_anchor":bool(global_anchor)})
                if saved_ok:
                    periodic_save_pending = False
                    periodic_save_due_clock = None
        if lazy:
        ...
        if sweep_enabled:
            rec.update({"descending_sweep_shared_batch":True,
                        "descending_sweep_cache":bool(model._descending_sweep_cache is not None),
                        "descending_sweep_safe_cache":bool(getattr(args, "descending_sweep_safe_cache", False)),
                        "sweep_active":bool(sweep_batch is not None),
                        "sweep_objective":("ar" if global_anchor else (sweep_objective if sweep_batch is not None else _ah_obj)),
                        "fresh_input_tokens_total":sweep_fresh_tokens,
                        "optimizer_exposure_tokens_total":sweep_exposure_tokens,
                        "stage_evaluations_total":sweep_stage_evals,
                        "fresh_batch_step":bool(fetched_fresh)})
    ...
    stream.stop_prefetch()
    _med_step = (statistics.median(step_times) if step_times else None)
    result={"schema":"agillm.gb10.1pf.canary.v1","ok":True,"steps":args.steps,"seen_tokens":seen,"median_step_s":_med_step,"median_tok_s":(None if _med_step is None else args.batch*args.seq/_med_step),"parameters":params,"target_training_tokens":TOTAL_TRAIN_TOKENS,"verified_sparse_microkernel_tflops":VERIFIED_SPARSE_TFLOPS,"finished_utc":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime())}
    if sweep_enabled:
        result.update({"descending_sweep_shared_batch":True,
                       "descending_sweep_cache":bool(getattr(args, "descending_sweep_cache", False)),
                       "descending_sweep_safe_cache":bool(getattr(args, "descending_sweep_safe_cache", False)),
                       "fresh_input_tokens":sweep_fresh_tokens,
                       "optimizer_exposure_tokens":sweep_exposure_tokens,
                       "stage_evaluations":sweep_stage_evals,
                       "accounting_note":"fresh tokens are stream positions; optimizer exposures count replayed local updates"})
    if sweep_enabled:
        def _fp(t):
            b=t.detach().contiguous().view(torch.uint8).cpu().numpy().tobytes()
            return hashlib.sha256(b).hexdigest()
        result["state_fingerprint"] = {
            "stage15_gate": _fp(model.stages[15].experts[0].gate.weight),
            "stage8_router": _fp(model.stages[8].router.weight),
            "stage0_down": _fp(model.stages[0].experts[0].down.weight),
            "stage15_q": _fp(model.stages[15].attn.q.weight),
            "stage0_q": _fp(model.stages[0].attn.q.weight),
            "head_mix": _fp(model.mix_head.weight),
        }
...
def main():
    ...
    t.add_argument("--descending-sweep-shared-batch",action="store_true",help="experimental same-batch 15->0 replay curriculum without prefix cache; A/B reference")
    t.add_argument("--descending-sweep-cache",action="store_true",help="experimental all-objective cache; NAT currently fails exactness gate")
    t.add_argument("--descending-sweep-safe-cache",action="store_true",help="exact hybrid: cache AR/SAT; NAT uses uncached shared-batch prefix recomputation")
