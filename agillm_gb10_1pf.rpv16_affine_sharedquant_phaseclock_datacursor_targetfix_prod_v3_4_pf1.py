...
DATA_PIPELINE_VERSION = "astra-prefetch-consumed-cursor-v1"
def _emit(record):
...
def train(args):
    ...
    active_opt = None
    active_params = None
    if not args.no_data_prefetch:
        stream.start_prefetch(args.batch,args.seq,depth=args.data_prefetch_depth,pin=True,gil_release=True)
    _pipeline_start=time.perf_counter(); _pipeline_seen0=seen; _pipeline_targets=0
    _emit({"event":"data_pipeline_ready","version":DATA_PIPELINE_VERSION,"prefetch":not args.no_data_prefetch,"depth":args.data_prefetch_depth,"resume_step":start_step-1,"checkpoint_cursor":"last_consumed_batch","math_unchanged":True})
    for step in range(start_step,args.steps+1):
        _wall_s0=time.perf_counter()
        if micro_in_update == 0:
        ...
        if do_update:
            ...
            if args.save_every_updates and update_number % args.save_every_updates == 0:
                ...
                if free < need:
                    ...
                    torch.save({"schema":("agillm.gb10.1pf.recovery.v5" if AH is None else ALLHEADS_CKPT_SCHEMA),"target_alignment":TARGET_ALIGNMENT,"step":step,"seen_tokens":seen + args.batch*args.seq,"model":model.state_dict(),"optimizer_bank":{"stages":[o.state_dict() for o in stage_opts],"head":head_opt.state_dict()},"optimizer_name":optimizer_name,"seed":args.seed,"optimizer_update_clock":optimizer_update_clock,"grad_accum":args.grad_accum,"micro_in_update":micro_in_update,"data_state":stream.consumed_state_dict(),**({} if AH is None else AH.ckpt_extra())},tmp)
                    os.replace(tmp,dst)
        ...
        _wall_end=time.perf_counter(); _wall_dt=_wall_end-_wall_s0
        _pipeline_targets+=int(rec.get("n_targets",args.batch*args.seq))
        rec.update({"data_pipeline":DATA_PIPELINE_VERSION,"wall_step_s":_wall_dt,"wall_tok_s":args.batch*args.seq/_wall_dt,"data_wait_s":_data_wait_s,"data_prefetch_qsize":stream.last_qsize,"pipeline_tok_s":(seen-_pipeline_seen0)/(_wall_end-_pipeline_start),"pipeline_target_s":_pipeline_targets/(_wall_end-_pipeline_start),"wall_time_s":_wall_end-_pipeline_start})
        _emit(rec)
    ...
    result.update({"data_pipeline":DATA_PIPELINE_VERSION,"pipeline_wall_s":_elapsed,"pipeline_tok_s":(seen-_pipeline_seen0)/_elapsed,"pipeline_target_s":_pipeline_targets/_elapsed})
...
def main():
    ...
    t.add_argument("--no-data-prefetch",action="store_true",help="Disable the exact consumed-cursor input prefetcher; model math is unchanged")
    t.add_argument("--data-prefetch-depth",type=int,default=3)
    a=p.parse_args()
