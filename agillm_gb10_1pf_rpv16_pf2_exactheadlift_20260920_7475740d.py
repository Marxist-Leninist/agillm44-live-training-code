...
def _rpv16_exact_logit_newton_direction(model, h, label, damping=1.0):
    """Exact float64 solve of the live RPV16 per-token product-vocabulary Hessian."""
    ...
    raw = raw_gpu.cpu().double()
    mix = mix_gpu.cpu().double()
    group = raw[:, :VOCAB_GROUPS]
    low = raw[:, VOCAB_GROUP_PAD:]
    pi = mix.softmax(0)
    q = group.softmax(1)
    slo = low.softmax(1)
    target = int(label.detach().cpu())
    aa = torch.tensor(VOCAB_AFFINE_A, dtype=torch.long)
    bb = torch.tensor(VOCAB_AFFINE_B, dtype=torch.long)
    ...
    gm = pi - alpha
    gg = alpha[:, None] * q
    gg[torch.arange(VOCAB_RANK), tg] -= alpha
    gl = alpha[:, None] * slo
    gl[torch.arange(VOCAB_RANK), tl] -= alpha
    grad = torch.cat((gm, gg.reshape(-1), gl.reshape(-1)))
    rhs = -grad
    ...
    V = torch.zeros((R, Dv), dtype=torch.float64)
    V[:, :R] = torch.eye(R, dtype=torch.float64)
    og=R; ol=R+R*G
    for rr in range(R):
        V[rr, og+rr*G:og+(rr+1)*G] = -q[rr]
        V[rr, og+rr*G+int(tg[rr])] += 1.0
        V[rr, ol+rr*L:ol+(rr+1)*L] = -slo[rr]
        V[rr, ol+rr*L+int(tl[rr])] += 1.0
    Calpha = torch.diag(alpha) - alpha[:,None] * alpha[None,:]
    def binv_apply(v):
        vm,vg,vl=unpack(v)
        ym=_rpv16_scaled_softmax_solve(pi,vm,1.0,damping)
        yg=torch.empty_like(vg); yl=torch.empty_like(vl)
        for rr in range(R):
            yg[rr]=_rpv16_scaled_softmax_solve(q[rr],vg[rr],alpha[rr],damping)
            yl[rr]=_rpv16_scaled_softmax_solve(slo[rr],vl[rr],alpha[rr],damping)
        return pack(ym,yg,yl)
    y0=binv_apply(rhs)
    W=torch.stack([binv_apply(V[rr]) for rr in range(R)],dim=1)
    M=V@W
    S=torch.eye(R,dtype=torch.float64)-Calpha@M
    z=torch.linalg.solve(S,Calpha@(V@y0))
    y=y0+W@z
    def bop(v):
        vm,vg,vl=unpack(v)
        om=_rpv16_cov_apply(pi,vm)+damping*vm
        ogv=torch.empty_like(vg); olv=torch.empty_like(vl)
        for rr in range(R):
            ogv[rr]=alpha[rr]*_rpv16_cov_apply(q[rr],vg[rr])+damping*vg[rr]
            olv[rr]=alpha[rr]*_rpv16_cov_apply(slo[rr],vl[rr])+damping*vl[rr]
        return pack(om,ogv,olv)
    Ay=bop(y)-V.T@(Calpha@(V@y))
...
def train(args):
    ...
    for step in range(start_step,args.steps+1):
        ...
        if do_update:
            ...
            active_opt.step()
            if global_anchor:
                rpv_exact_head_receipt = rpv16_exact_headlift_correction(model, damping=1.0)
