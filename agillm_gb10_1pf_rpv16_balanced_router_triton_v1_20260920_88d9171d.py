...
import torch
from sg_balanced_router_nll_v1 import capped_selected_nll6
...
class Stage(nn.Module):
    ...
    def forward(self, x):
        ...
        _balanced_route = (flat.shape[0] % EXPERTS == 0)
        if _balanced_route:
        ...
        if _balanced_route:
            # Lean-checked exact real identity; specialized six-class Triton
            # forward/backward preserves the balanced discrete route and avoids
            # materializing a generic probability tensor.
            aux = capped_selected_nll6(logits, route)
        else:
